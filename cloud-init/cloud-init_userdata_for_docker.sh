...
echo cloud-init RHEL Network Settings Start `date` >> ${LOGFILE}
/sbin/service network status >> ${LOGFILE}
...
/sbin/service network restart >> ${LOGFILE}
/sbin/service network status >> ${LOGFILE}
echo cloud-init RHEL Network Settings Complete `date` >> ${LOGFILE}
...
echo "other_args=\"--debug=true --bridge=br0 --dns=${DNS_IP} --icc=true\"" >> /etc/sysconfig/docker
